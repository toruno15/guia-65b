var fila="<tr><td class='id'></td><td class='foto'></td><td class='price'></td><td class='title'></td><td class='description'></td><td class='category'></td></tr>";
  ...
  function codigoCat(catstr) {
	...
	function listarProductos(productos) {
		...
		ident.setAttribute("onclick", "orden*=-1;listarProductos(productos);");
	  ...
	  if(orden===0) {orden=-1;ident.innerHTML="id"}
	  else
	     if(orden==1) {
			 ...
			 ordenarAscID(productos,"id");
			 ident.innerHTML="ID A";
			 ident.style.color="darkgreen"
			}
	     else 
	       if(orden==-1) {
			   ...
			   //ordenar de forma decendente los ids
			   ordenarDescID(productos,"id");
			   ident.innerHTML="ID D";
...
function ordenarDescID(p_array_json, p_key) {
	p_array_json.sort(function (a, b) {
	   if(a[p_key] > b[p_key]) return -1;
 if(a[p_key] < b[p_key]) return 1;
 return 0;
 ...
 function ordenarAscID(p_array_json, p_key) {
	p_array_json.sort(function (a, b) {
	   if(a[p_key] > b[p_key]) return 1;
 if(a[p_key] < b[p_key]) return -1;
 return 0;
